...
export default function ModalDelete({
  ...
  async function handleAction() {
    const res = await fetch(`/api/assinatura-email/setores/${id}`, {
      method: "PATCH",
    ...
    toast.success(status ? "Unidade ativada com sucesso" : "Unidade desativada com sucesso");
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{status ? "Ativar Unidade" : "Desativar Unidade"}</DialogTitle>
        </DialogHeader>
        <p>
          {status
            ? "Tem certeza que deseja ativar esta unidade?"
