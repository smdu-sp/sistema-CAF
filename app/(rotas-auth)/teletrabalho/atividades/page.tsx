...
import { colunasAtividades } from "./_components/columns";
...
export default async function AtividadesPage({ searchParams }: PageProps) {
  ...
  const [lista, total, unidades, categorias, cargos] = await Promise.all([
    prisma.ttAtividade.findMany({
    ...
    prisma.ttAtividade.count(),
    prisma.ttUnidade.findMany({ where: { ativo: true }, select: { id: true, sigla: true, nome: true }, orderBy: { nome: "asc" } }),
    prisma.ttCategoriaAtividade.findMany({ select: { id: true, nome: true, unidadeId: true }, orderBy: { ordem: "asc" } }),
  ...
  return (
    <div className="w-full px-0 md:px-8 relative pb-20 md:pb-14 h-full md:container mx-auto">
      <div className="flex flex-col max-w-sm mx-auto md:max-w-full gap-3 my-5 w-full">
        <DataTable
          columns={colunasAtividades(unidades, categorias, cargos)}
          data={lista}
          paginaAtual={pagina}
          limitePorPagina={limite}
          totalItens={total}
          labelItemSingular="atividade"
          labelItemPlural="atividades"
        />
      </div>
      <div className="absolute bottom-10 md:bottom-5 right-2 md:right-8 hover:scale-110">
        <ModalUpdateAndCreate isUpdating={false} unidades={unidades} categorias={categorias} cargos={cargos} />
      </div>
    </div>
  );
}
