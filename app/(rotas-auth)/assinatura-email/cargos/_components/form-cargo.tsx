...
export default function FormCargo({ isUpdating, cargo }: FormCargoProps) {
  const [isPending, startTransition] = useTransition();
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: { nome: cargo?.nome ?? "" },
  });

  async function onSubmit(values: z.infer<typeof formSchema>) {
    startTransition(async () => {
      try {
        if (isUpdating && cargo?.id) {
          const res = await fetch(`/api/assinatura-email/cargos/${cargo.id}`, {
            method: "PATCH",
          ...
          const res = await fetch("/api/assinatura-email/cargos", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ nome: values.nome.trim() }),
          });
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Ex: ANALISTA DE SISTEMA" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
