...
import { colunasUnidades } from "./_components/columns";
...
export default async function UnidadesTeletrabalhoPage({ searchParams }: PageProps) {
  ...
  const [lista, total, servidores] = await Promise.all([
    ...
    prisma.ttUnidade.count(),
    prisma.ttServidor.findMany({
      where: { ativo: true },
      select: { id: true, nome: true, rf: true },
      orderBy: { nome: "asc" },
    }),
  ]);

  const todasUnidades = await prisma.ttUnidade.findMany({
  ...
  return (
    <div className="w-full px-0 md:px-8 relative pb-20 md:pb-14 h-full md:container mx-auto">
      <div className="flex flex-col max-w-sm mx-auto md:max-w-full gap-3 my-5 w-full">
        <DataTable
          columns={colunasUnidades(todasUnidades, servidores)}
          data={lista}
          paginaAtual={pagina}
          limitePorPagina={limite}
          totalItens={total}
          labelItemSingular="unidade"
          labelItemPlural="unidades"
        />
      </div>
      <div className="absolute bottom-10 md:bottom-5 right-2 md:right-8 hover:scale-110">
        <ModalUpdateAndCreate isUpdating={false} unidades={todasUnidades} servidores={servidores} />
      </div>
    </div>
  );
}
